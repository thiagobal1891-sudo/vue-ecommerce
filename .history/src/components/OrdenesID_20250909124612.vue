...
export default {
  ...
  setup() {
    ...
    const filteredProductos = computed(() => {
      return cartStore.productos.filter(producto => {
        if (selectedTab.value === "Completed" && producto.status !== "Paid") return false
        if (selectedTab.value === "Processing" && producto.status !== "Processing") return false
        if (selectedTab.value === "Cancelled" && producto.status !== "Cancelled") return false

        if (searchQuery.value) {
          return (
            producto.id.toString().includes(searchQuery.value) ||
            producto.date.toLowerCase().includes(searchQuery.value.toLowerCase()) ||
            producto.price.includes(searchQuery.value)
          )
        }
        return true
      })
