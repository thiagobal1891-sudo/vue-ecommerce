...
// Convertimos el cart a un formato de orden
const orders = computed(() => {
  return cartStore.cart.map(producto => ({
    id: producto.id,
    date: 'September 5, 2020',
    price: producto.price,
    status: 'Paid' // podrías cambiar esto dinámicamente si tienes lógica
  }))
})
...
const filteredOrders = computed(() => {
  return orders.value.filter(order => {
    const matchesTab = 
      (selectedTab.value === 'Completed' && order.status === 'Paid') ||
      (selectedTab.value === 'Processing' && order.status === 'Processing') ||
      (selectedTab.value === 'Cancelled' && order.status === 'Cancelled')

    const matchesSearch = order.id.toString().includes(searchQuery.value.toLowerCase()) ||
                          order.date.toLowerCase().includes(searchQuery.value.toLowerCase()) ||
                          order.price.toString().includes(searchQuery.value.toLowerCase())
